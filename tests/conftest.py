...
def pytest_sessionfinish(session, exitstatus):
    if os.path.isdir("./src/main/yang"):
        # First line is the grand total; the rest are one line per
        # first-level directory, not broken down any further.
        shell(("du -h --max-depth=1 ./src/main/yang 2>/dev/null | sort -rh",))
